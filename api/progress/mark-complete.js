...
module.exports = async (req, res) => {
  ...
  const type = String(body.type || '') // 'lesson' | 'module' | 'course'
  ...
  if (type === 'lesson') {
    const { error } = await supabase
      ...
      .upsert({ user_id: userId, lesson_id: id, completed_at: nowIso, marked_by: actorId, marked_at: nowIso }, { onConflict: 'user_id,lesson_id' })
    if (error) return json(res, 500, { ok: false, error: error.message })
  } else if (type === 'module') {
    const { error } = await supabase
      .from('module_completions')
      .upsert({ user_id: userId, module_id: id, completed_at: nowIso, marked_by: actorId, marked_at: nowIso }, { onConflict: 'user_id,module_id' })
    if (error) return json(res, 500, { ok: false, error: error.message })
  } else if (type === 'course') {
    const { error } = await supabase
      .from('course_completions')
      .upsert({ user_id: userId, course_id: id, completed_at: nowIso, marked_by: actorId, marked_at: nowIso }, { onConflict: 'user_id,course_id' })
    if (error) return json(res, 500, { ok: false, error: error.message })
  } else {
